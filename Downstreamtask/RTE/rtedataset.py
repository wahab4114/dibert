...
class RTEdataset(Dataset):
    def __init__(self, split_name = 'validation'):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(rteConfig.tokenizer_name)

    def __getitem__(self, item):
        t1, t2, label = self.sen1[item], self.sen2[item], self.label[item]
        encoded = self.tokenizer([t1],[t2],max_length=rteConfig.seq_len, padding="max_length", truncation=True, return_tensors='pt')
    ...
    def read_tsv(self, split_name='validation'):
        dataset = load_dataset('glue','rte', split=split_name)
        return dataset['sentence1'], dataset['sentence2'], dataset['label']


if __name__ == '__main__':
    split_name = 'train'
    #print(read_tsv('validation'))
    data = RTEdataset(split_name)
    print(len(data))
    print(data[0])
