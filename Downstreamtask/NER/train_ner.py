...
from Downstreamtask.NER.nermodel import Bert_ner
import tqdm
import random
import optuna

from torch.utils.data import Dataset, DataLoader
from Downstreamtask.NER.nerdataset import Data_preprocessing,EntityDataset
...
def training(model, optim, criterion_cls, train_iter, epoch):
    ...
    for batch in tqdm.tqdm(train_iter):
        ...
        loss_pos = criterion_cls (logits_pos.view(-1, NERConfig.num_labels_pos), truelabel_pos.view(-1,))
        loss = loss_pos
...
def train_val(train_data, valid_data, model_path:str,trial=None, best_params=None):
    ...
    model = Bert_ner(pretrained_model= bert_pretrained, hidden_out=NERConfig.hidden_model_out, drop_out= drop_out)
    ...
    for epoch in range(epochs):
        ...
        if(trial is None and best_params is not None):
            print('-saving model-')
            torch.save(model, 'results/full_text/dibert_NER_mlm_cls_pprediction_103_10_seed_'+str(2)+'_epoch_'+str(epoch+1)+'.tar')
...
if __name__ == '__main__':
    param_path = 'results/full_text/params/dibert_NER_mlm_cls_pprediction_best.json'
    model_path = 'results/dibert_NER_mlm_cls_29_seed_'+str(0)+'.tar'

    is_tune = False
    #np.random.seed(NERConfig.seed_3)
    #torch.manual_seed(NERConfig.seed_3)
    #random.seed(NERConfig.seed_3)
    #torch.cuda.manual_seed_all(NERConfig.seed_3)
    ...
    train_data_loader = DataLoader(train_data, batch_size=NERConfig.batch_size)
    valid_data_loader = DataLoader(valid_data, batch_size=NERConfig.batch_size)
    ...
    if(is_tune == False):
        ...
        print_result(score, NERConfig.epochs)
    elif(is_tune == True):
        print(param_path)
