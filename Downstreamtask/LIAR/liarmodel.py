...
class ClassificationHead(nn.Module):
    def __init__(self, hidden_out , drop_out):
        ...
        self.cls_layer = nn.Linear(self.hidden, 6)
    def forward(self, x):
        x = self.drop_out(x)
        x = self.cls_layer(x)
        return x
...
class Bert_liar(nn.Module):
    def __init__(self, pretrained_model, hidden_out = liarConfig.hidden_model_out, seq_len = liarConfig.seq_len, drop_out = liarConfig.drop_out):
        super().__init__()
