...
from Downstreamtask.SciTail.scitailmodel import Bert_scitail
import tqdm
import random
import optuna
...
def train_val(train_data, valid_data, model_path:str,trial=None, best_params=None):
    ...
    if (best_params is not None):
        ...
        drop_out = scitailConfig.drop_out
    ...
    model = Bert_scitail(pretrained_model= bert_pretrained, hidden_out=scitailConfig.hidden_model_out, drop_out= drop_out)
    ...
    for epoch in range(epochs):
        ...
        if(trial is None and best_params is not None):
            print('-saving model-')
            torch.save(model, 'results/full_text/dibert_SCI_mlm_cls_103_10_seed_'+str(4)+'_epoch_'+str(epoch+1)+'.tar')
...
if __name__ == '__main__':
    param_path = 'results/full_text/params/dibert_SCI_mlm_cls_10_best.json'
    ...
    train_path = 'data/scitail_1.0_train.tsv'
    valid_path = 'data/scitail_1.0_dev.tsv'
    test_path = 'data/scitail_1.0_test.tsv'

    train_data = SciTailData('train')
    valid_data = SciTailData('validation')
    ...
    train_data_loader = DataLoader(train_data, batch_size=scitailConfig.batch_size)
    valid_data_loader = DataLoader(valid_data, batch_size=scitailConfig.batch_size)
    ...
    if(is_tune == False):
        ...
        print_result(score, scitailConfig.epochs)
    elif(is_tune == True):
        print(param_path)
