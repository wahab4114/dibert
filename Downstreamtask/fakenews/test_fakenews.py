...
def main():
    ...
    test_data = FakeNewsDataset(test_file)
    print(len(test_data))
    fakenews_bert = torch.load(model_path)
    test_iter = DataLoader(test_data, batch_size=fakenewsConfig.batch_size, shuffle=False)

    label, preds = testing(test_iter, fakenews_bert)
