...
from Downstreamtask.fakenews.fakenewsmodel import Bert_fakenews
import tqdm
import random
import optuna

from torch.utils.data import Dataset, DataLoader
from Downstreamtask.fakenews.fakenewsdataset import FakeNewsDataset
...
def train_val(train_data, valid_data, model_path:str,trial=None, best_params=None):
    ...
    if (best_params is not None):
        ...
        drop_out = fakenewsConfig.drop_out
    ...
    model = Bert_fakenews(pretrained_model= bert_pretrained, hidden_out=fakenewsConfig.hidden_model_out, drop_out= drop_out)
...
if __name__ == '__main__':
    ...
    #torch.cuda.manual_seed_all(imdbConfig.seed_3)
    ...
    train_data_loader = DataLoader(train_data, batch_size=fakenewsConfig.batch_size)
    valid_data_loader = DataLoader(valid_data, batch_size=fakenewsConfig.batch_size)
    ...
    if(is_tune == False):
        ...
        print_result(score, fakenewsConfig.epochs)
    elif(is_tune == True):
        print(param_path)
