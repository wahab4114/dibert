...
from Downstreamtask.QNLI.qnlimodel import Bert_qnli
import tqdm
import random
import optuna
...
def train_val(train_data, valid_data, model_path:str,trial=None, best_params=None):
    ...
    model = Bert_qnli(pretrained_model= bert_pretrained, hidden_out=qnliConfig.hidden_model_out, drop_out= drop_out)
    ...
    for epoch in range(epochs):
        ...
        if(trial is None and best_params is not None):
            print('-saving model-')
            torch.save(model, 'results/full_text/dibert_QNLI_mlm_cls_pprediction_103_10_seed_'+str(4)+'_epoch_'+str(epoch+1)+'.tar')
...
if __name__ == '__main__':
    param_path = 'results/full_text/params/dibert_QNLI_mlm_cls_pprediction_10_best.json'
    model_path = 'results/dibert_QNLI_mlm_cls_pp_29_seed_'+str(0)+'.tar'
    ...
    train_data = QNLIdataset('train')
    valid_data = QNLIdataset('validation')
    ...
    train_data_loader = DataLoader(train_data, batch_size=qnliConfig.batch_size)
    valid_data_loader = DataLoader(valid_data, batch_size=qnliConfig.batch_size)
    ...
    if(is_tune == False):
        ...
        print_result(score, qnliConfig.epochs)
    elif(is_tune == True):
        print(param_path)
